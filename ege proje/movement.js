...
export class Movement {
  ...
  _playAnimationByIndex(index) {
    ...
    if (clip) {
      ...
      console.error(`animasyon bulunamadı. hatalı index : ${index}.`);
    }
  }
  ...
  _updateVelocity(timeInSeconds) {
    ...
    if (this._input._keys.isSpaceDown && !this._isJumping) {
      velocity.y += 10;
      this._isJumping = true;
    }
    if (this._isJumping) {
      const gravity = 9.8;
      velocity.y -= gravity * timeInSeconds;
    }
  }
  ...
  _updatePosition(timeInSeconds) {
    ...
    const upwards = new THREE.Vector3(0, 1, 0).applyQuaternion(controlObject.quaternion).normalize();

    sideways.multiplyScalar(this._velocity.x * timeInSeconds);
    forward.multiplyScalar(this._velocity.z * timeInSeconds);
    upwards.multiplyScalar(this._velocity.y * timeInSeconds);

    const newPosition = controlObject.position.clone().add(forward).add(sideways).add(upwards);

    if (newPosition.y < 0) {
      controlObject.position.set(controlObject.position.x, 0, controlObject.position.z);
      this._velocity.y = 0;
      this._isJumping = false;
    } else {
